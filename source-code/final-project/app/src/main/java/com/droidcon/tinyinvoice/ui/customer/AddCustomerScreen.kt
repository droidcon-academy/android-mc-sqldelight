...
@Composable
fun AddCustomerRoute(
    viewModel: CustomerViewModel = hiltViewModel(),
) {
    val context = LocalContext.current
    val activity = context as? Activity
    AddCustomerScreen(
        addNewCustomer = { name, address, phone, email ->
            viewModel.addNewCustomer(name, address, phone, email)
...
fun AddCustomerScreen(
    addNewCustomer: (name: String, address: String, phone: String, email: String) -> Unit
) {
...
@ThemePreviews
@Composable
private fun AddCustomerScreenPrev() {
    TinyInvoiceTheme {
        Surface {
            AddCustomerScreen(
                addNewCustomer = { _, _, _, _ -> }
            )
        }
    }
}